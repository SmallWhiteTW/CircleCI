<template>
          ...
          <div class="card-body px-0 pb-2">
            ...
            <div class="table-responsive p-0">
              <table class="table align-items-center mb-0">
                ...
                <tbody>
                  <tr
                    v-for="category in state.categoriesData"
                    :key="category.ACategoryID"
                  >
                    <td>
                      <div class="d-flex px-2 py-1">
                        <div class="d-flex flex-column justify-content-center">
                          <span>{{ category.ACategoryName }}</span>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      <button
                        ...
                        style="margin: 3px"
                        @click="editData(category.ACategoryID)"
                      >
                        {{ t("Categories.Edit") }}
                      </button>
                    </td>
  ...
  <el-dialog v-model="state.showDialog" style="width: 500px">
    ...
    <el-form :model="editor" label-width="200px">
      <el-form-item
        :label="t('Categories.CategoriesName')"
        :label-width="formLabelWidth"
      >
        <el-input
          v-model="editor.CategoriesName"
          autocomplete="off"
      ...
      <el-form-item :label="t('Categories.Slug')" :label-width="formLabelWidth">
        <el-input v-model="editor.Slug" autocomplete="off" />
      </el-form-item>
    </el-form>
...
const editData = (ACategoryID) => {
  state.isUpdate = true;
  state.showDialog = true;
  let ctg = state.categoriesData.filter(
    (x) => x.ACategoryID === ACategoryID
  )[0];
  editor.CategoryID = ACategoryID;
  editor.CategoriesName = ctg.ACategoryName;
  editor.Description = ctg.Description;
  editor.Slug = ctg.Slug;
};

const cancel = () => {
  editor.CategoryID = "";
  editor.CategoriesName = "";
  editor.Description = "";
  editor.Slug = "";
...
const confirm = async () => {
  let payload = {
    ACategoryName: editor.CategoriesName,
    Description: editor.Description,
    Slug: editor.Slug,
  };
  if (state.isUpdate) {
    payload.ACategoryID = editor.CategoryID;
  }
  const { IsSuccess, Msg } = await backofficeReq.updateArticleCategoryList(payload);
  if (IsSuccess) {
